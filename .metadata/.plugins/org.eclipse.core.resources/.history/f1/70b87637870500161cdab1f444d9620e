...
public class AgykesleltetesFo extends JFrame{
	private static final long serialVersionUID = 1L;
	
	public static void main(String arg[]) {
		new AgykesleltetesFo();

	}
	public AgykesleltetesFo() {
		 setTitle("Akasztófa - Horváth Dávid");
		Toolkit tk=Toolkit.getDefaultToolkit();
		int x=(int) tk.getScreenSize().getWidth();
		int y =(int)tk.getScreenSize().getHeight();
		setSize(x, y);
		setDefaultCloseOperation(EXIT_ON_CLOSE);
		AgykesleltetesGraf t = new AgykesleltetesGraf();
		add(t);
		setUndecorated(true);
		setResizable(false);
		setVisible(true);
